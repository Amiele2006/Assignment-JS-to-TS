class Mimi {
    ...
    constructor(name:string, age:number, height:number, eyes:string[], fingers:string[]){
        this.name = name;
        this.age = age;
        this.height = height;
        this.eyes = eyes;
    ...
    join(arr: string[], seperator: string) {
    ...
    delete(arr:string[], index:number){
       if (index >= 0 && index < arr.length) {
          arr[index] = undefined as unknown as string; // Simulates a "hole" (like `delete` would)
        }
    }

    splice(arr: string[], start: number, deleteCount: number, ...items: string[]): string[] {
    ...
    slice(arr: string[], start: number, end?: number):string[] {
    const result: string[] = [];
    ...
    unshift(arr: string[], ...items: string[]): number {
